...
contract L2DebtManagerTest is Test {
    ...
    function setUp() public {
        ...
        l2DebtManager = new L2DebtManager(address(eEth), address(usdc), etherFiCashSafe);
    ...
    function test_borrowUSDC() public {
        ...
        uint256 beforeRemainingBorrowingCapacity = l2DebtManager.remainingBorrowingCapacityInUSDC(alice);
        ...
        assertEq(usdc.balanceOf(address(l2DebtManager)), beforeUsdcBalance - borrowAmount);
        assertEq(l2DebtManager.borrowingOf(alice), borrowAmount);
        assertEq(usdc.balanceOf(etherFiCashSafe), beforeEtherFiCashSafeBalance + borrowAmount);
        assertEq(l2DebtManager.remainingBorrowingCapacityInUSDC(alice), beforeRemainingBorrowingCapacity - borrowAmount);
    }
    ...
    function test_repayWithUSDC() public {
        ...
        assertEq(l2DebtManager.liquidUsdcAmount(), beforeLiquidUsdcAmount + 2 * repayAmount);
        assertEq(l2DebtManager.liquidEEthAmount(), beforeLiquidEEthAmount);

        assertEq(l2DebtManager.totalBorrowingAmount(), beforeBorrowing - repayAmount);
        assertEq(usdc.balanceOf(address(l2DebtManager)), beforeUsdcBalance + repayAmount);
        assertEq(l2DebtManager.borrowingOf(alice), beforeBorrowing - repayAmount);
    }
    ...
    function test_repayWithEETH() public {
        ...
        uint256 repayAmount = 500 * 1e6; // 500 USDC
        uint256 repayAmountInEEth = l2DebtManager.getCollateralAmountForDebt(repayAmount);
        l2DebtManager.repayWithEETH(repayAmount);

        assertEq(l2DebtManager.liquidUsdcAmount(), beforeLiquidUsdcAmount + repayAmount);
        assertEq(l2DebtManager.liquidEEthAmount(), beforeLiquidEEthAmount + repayAmountInEEth);

        assertEq(l2DebtManager.totalBorrowingAmount(), beforeBorrowing - repayAmount);
        assertEq(usdc.balanceOf(address(l2DebtManager)), beforeUsdcBalance);
        assertEq(l2DebtManager.borrowingOf(alice), beforeBorrowing - repayAmount);
        assertEq(l2DebtManager.collateralOf(alice), beforeCollateral - repayAmountInEEth);
    }
    ...
    function test_liquidation() public {
        ...
        assertEq(l2DebtManager.remainingBorrowingCapacityInUSDC(alice), 300 * 1e6); // 60% of 500 USDC = 300 USDC
    ...
    function test_borrow_above_liquidation_threshold_fails() public {
        ...
        assertGt(borrowAmount, l2DebtManager.remainingBorrowingCapacityInUSDC(alice));
